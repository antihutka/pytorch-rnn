...
optimizer = BetterAdamW(pgroups, lr=args.learning_rate, weight_decay=args.weight_decay, grad_clip=args.grad_clip, eps=args.eps, betas=(0.95, 0.999))
scheduler = optim.lr_scheduler.StepLR(optimizer, args.lrdecay_every, args.lrdecay_factor)
scheduler_start = optim.lr_scheduler.LinearLR(optimizer, start_factor=.01, total_iters=args.warmup_iters)
...
for epoch in range(0, args.num_epochs):
  ...
  for iter_data in traindata.data:
    ...
    if iter_data.i % args.print_every == 0:
      ...
      s += "%.1f MiB allocated" % (torch.cuda.max_memory_allocated(device=device)/1024/1024)
